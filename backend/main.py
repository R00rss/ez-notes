...
async def upload_file(
    ...
    try:
        ...
        if current_user["status"] == 1:
            ...
            aux_path = "./static/images/" + file.filename
            # aux_file = open(aux_path, "wb")
            # aux_file.write(contents)
            # aux_file.close()
            image = crud.add_image(
                db,
                image=schemasDB.ImageCreate(
                    note_id=id_note, path_file=aux_path, name=file.filename
                ),
            )
            pathToSave = path.join(
                pathname, "files", "images", f"note_{id_note}_img_{image.id}"
            )
            path_name = await manage_files.save_file(pathToSave, file)
